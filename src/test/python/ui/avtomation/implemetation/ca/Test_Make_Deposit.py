...
class DepositTestCa(BaseTest):
    ...
    def test_make_deposit_from_ca(self):
        BrandHomePage().open_first_tab_page(Config.url_new_forex) \
            ...
            .click_login_button() \
            .open_drop_down_menu() \
            .select_module(CaStatusConstants.DEPOSIT)

        deposit_successful = CADeposit().select_payment_method(CaStatusConstants.VISA) \
            .set_amount_deposit(CaStatusConstants.AMOUNT) \
        ...
        assert deposit_successful == CaStatusConstants.SUCCESSFUL_DEPOSIT_MESSAGE
        assert amount_successful == CaStatusConstants.SUCCESSFUL_AMOUNT_MESSAGE
