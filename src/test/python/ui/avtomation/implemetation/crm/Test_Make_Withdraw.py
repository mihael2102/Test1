...
class WithdrawTestCRM(BaseTest):
    ...
    def test_make_withdraw_crm(self):
        ...
        total_amount_withdraw_crm = CRMClientProfilePage() \
            .get_total_amount_withdraw_text(amount_initial, CRMConstats.AMOUNT_WITHDRAW)

        crm_client_profile.perform_scroll_up() \
            .open_mt4_actions(CRMConstats.WITHDRAW)

        amount_withdraw_crm = MT4Withdraw().make_withdraw(account, CRMConstats.AMOUNT_WITHDRAW) \
            .refresh_page() \
