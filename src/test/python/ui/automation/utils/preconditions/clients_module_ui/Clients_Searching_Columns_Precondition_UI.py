...
class ClientsSearchingColumnsPreconditionUI(object):
    ...
    def clients_searching_columns_ui(self):
        ...
        crm_id = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_CRM_ID,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        client_status = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_CLIENT_STATUS,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        email = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_EMAIL,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        client_name = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_CLIENT_NAME,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        assigned_to = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_ASSIGNED_TO,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        country = CRMBaseMethodsPage(self.driver) \
            .get_data_from_list_view_ui(column=ClientsModuleConstants.COLUMN_COUNTRY,
                                        row=ClientsModuleConstants.ROW_NUMBER_FOR_DATA_SEARCHING_1)
        ...
        if client_name:
            search\
                .set_data_column_field(column=ClientsModuleConstants.COLUMN_CLIENT_NAME,
                                       data=client_name)
        if client_status:
            search\
                .select_data_column_field(column=ClientsModuleConstants.COLUMN_CLIENT_STATUS,
                                          data=client_status)
        if assigned_to:
            search\
                .select_data_column_field(column=ClientsModuleConstants.COLUMN_ASSIGNED_TO,
                                          data=assigned_to)
        if country:
            search\
                .select_data_column_field(column=ClientsModuleConstants.COLUMN_COUNTRY,
                                          data=country)
        if email and ("*" not in email):
            search\
                .select_data_column_field(column=ClientsModuleConstants.COLUMN_EMAIL,
                                          data=email)
