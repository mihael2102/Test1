...
class ChangePasswordTestCRM(BaseTest):
    ...
    def test_change_mt4_password_from_crm(self):
        ...
        MT4DropDown(self.driver).open_mt4_module(MT4ModuleConstants.CHANGE_MT_PASSWORD)

        MT4UpdatePasswordModule(self.driver).select_account(account_number) \
                                            .enter_password(self.config.get_data_client(
                                                TestDataConstants.CLIENT_ONE, TestDataConstants.NEW_PASSWORD)) \
                                            .click_check_button()
        ...
        # change the mt password back to the original password
        crm_client_profile.refresh_page()
        MT4DropDown(self.driver).open_mt4_module(MT4ModuleConstants.CHANGE_MT_PASSWORD)
